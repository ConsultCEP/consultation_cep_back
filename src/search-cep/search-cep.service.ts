...
export class SearchCepService {
    ...
    async findAddresByCep(cep: string): Promise<string> {
        const data = this.httpService.axiosRef.get(`https://viacep.com.br/ws/${cep}/json/`)
            .then(({ data }) => {
                if (data.erro === 'true') {
    ...
    async findAddres({ uf, cidade, logradouro }: SearchAddres): Promise<AxiosResponse<SearchAddresByCep>> {
        const data = this.httpService.axiosRef.get(`https://viacep.com.br/ws/${uf}/${cidade}/${logradouro}/json/`)
            .then((e) => {
                if (e.data.erro === 'true') {
